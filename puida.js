...
let test = setInterval(() => {
  let elem = document.getElementsByClassName('a3s aXjCH m15b09dfbbecb0531');
  let elem2 = document.getElementsByClassName('ii gt adP adO');
  if (!elem2[0]) return;
  if (elem2[0].innerHTML.indexOf('ДЕ Розділ 1 ???') !== -1 && elem2[0].innerHTML.indexOf('Що за пиздобольство?') === 1) {
    let el = elem2[0];
    let index = elem2[0].innerHTML.indexOf('ДЕ Розділ 1 ???');
    el.innerHTML = [el.innerHTML.slice(0, index), 'Що за пиздобольство? ', el.innerHTML.slice(index)].join('');
  }
  console.log(elem);
  for (let i = 0; i < elem.length; ++i) {
    let el = elem[i].getElementsByTagName('span');
    if (!el) return;
    for (let j = 0; j < el.length; ++j) {
      if (el[j].innerHTML.indexOf('Що за пиздобольство?') === -1) {
        let index = el[j].innerHTML.indexOf('ДЕ Розділ 1 ???');
        if (index !== -1) {
