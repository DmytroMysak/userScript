...
(function () {
  ...
  function filterlogic() {
    let els = contains('div[id^=hyperfeed_story_id]', 'Suggested Post');
    for (var el in els) {
      els[el].outerHTML = '';
    }
...
(function () {
  ...
  const nodeContentKey = (('innerText' in document.documentElement) ? 'innerText' : 'textContent');
  ...
  function process() {
    ...
    const stories = stream.querySelectorAll(storySelector);
    if (!stories.length) {
      return;
    }
