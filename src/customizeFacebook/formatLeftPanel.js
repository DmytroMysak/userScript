...
const alwaysFixLeftMenu = () => {
	...
	return leftPanel ? _recursiveWaitForCorrectNodeElement(leftPanel) : null;
};

const _recursiveWaitForCorrectNodeElement = elem => {
	if (elem.parentNode.classList.length && !elem.parentNode.classList.contains('custom-fixed-class-left-panel')) {
		setTimeout(() => _recursiveWaitForCorrectNodeElement(elem), 100);
	} else {
